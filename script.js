...
function project({principal,dailyPct,days,reinvest,withdrawEvery,reinvestCap}){
  ...
  for(let d=1; d<=days; d++){
    ...
    if(!reinvest){
      ...
      if(reinvestCap>0 && balance>=reinvestCap){
        withdrawn = profit;
        totalWithdrawn += withdrawn;
      }else{
        balance += profit;
    ...
    if(withdrawEvery>0 && d%withdrawEvery===0){
      // hook for extra withdraws if needed
    }
...
function renderHotList(){
  ...
  coins.forEach(c=>{
    ...
    row.innerHTML=`<div class="flex items-center gap-3">
        ...
        <div><div class="font-medium">${c.s} <span class="text-xs text-slate-500">USDT</span></div></div>
      </div>
...
function setupCharts(){
  ...
  miniChart = new Chart(mctx, {
    type:'line',
    data:{labels:Array.from({length:20},(_,i)=>i+1), datasets:[{label:'Growth', data:Array.from({length:20},(_,i)=> (i*i/2)+Math.random()*5), tension:.2, fill:true}]},
